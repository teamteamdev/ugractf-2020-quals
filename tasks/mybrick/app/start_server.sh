...
if [[ ! -d $1/mongo ]]; then
    ...
    for i in seq 1 5; do
        sleep 1
        if mongo --host $1/mongo.sock db --authenticationDatabase admin --eval "db.createUser({user: \"$MONGO_USER\", pwd: \"$MONGO_PASS\", roles: [{role: \"userAdminAnyDatabase\", db: \"db\"}]})"; then
            DONE=1
            break
        fi
    done
