...
void LoadBoard( BOARD *p_board )
{
    ...
    if( RegQueryValueExW( hkey, minesW, NULL, &type, (BYTE*) &p_board->mines, &size ) )
        // FIXME FIXME
        // p_board->mines = p_board->rows * p_board->cols - 1;
        p_board->mines = p_board->rows * (p_board->cols - 1);
...
void SetTheFirstMine ( BOARD *p_board, int selected_col, int selected_row )
{
...
static void PlaceMines ( BOARD *p_board, int selected_col, int selected_row )
{
    ...
    SetTheFirstMine(p_board, selected_col, selected_row);
...
static void TestBoard( HWND hWnd, BOARD *p_board, int x, int y, int msg )
{
    ...
    if( p_board->boxes_left == 0 && p_board->mines < p_board->rows * p_board->cols ) {
        p_board->status = WON;
        ...
        if( p_board->difficulty != CUSTOM &&
            ...
            mbstowcs(p_board->best_name[2], get_best_name(), sizeof(p_board->best_name[2]) / sizeof(WCHAR));
...
int WINAPI WinMain( HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR cmdline, int cmdshow )
{
    ...
    HICON icon = LoadIconW( hInst, MAKEINTRESOURCEW(IDI_WINEMINE) );
    if (icon == NULL) {
        ExitProcess(1);
    }
